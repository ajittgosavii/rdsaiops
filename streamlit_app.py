...
class StreamlitClaudeAIAnalyzer:
    ...
    """AI-powered network transfer analysis using Claude API"""
    
    def __init__(self, anthropic_api_key: Optional[str] = None):
        self.anthropic_api_key = anthropic_api_key
        self.client = None
        
        # Initialize transfer patterns for context
        self.transfer_patterns = {
            'internet_dms': {
                'name': 'Internet + DMS',
                'description': 'Standard internet with AWS Database Migration Service',
                'typical_cost_per_gb': 0.09,
                'setup_cost': 2000,
            ...
            'dx_dms': {
                ...
                'description': 'Dedicated connection with DMS',
                'typical_cost_per_gb': 0.02,
                'setup_cost': 8000,
            ...
            'dx_datasync_vpc': {
                'name': 'Direct Connect + DataSync + VPC',
                'description': 'DX with DataSync and VPC endpoints for maximum security',
                'typical_cost_per_gb': 0.0125,
                'setup_cost': 12000,
            ...
            'vpn_dms': {
                'name': 'VPN + DMS',
                'description': 'Site-to-site VPN with DMS',
                'typical_cost_per_gb': 0.09,
                'setup_cost': 3000,
            ...
            'hybrid_snowball_dms': {
                ...
                'description': 'Initial bulk via Snowball, sync with DMS',
                'typical_cost_per_gb': 0.01,
                'setup_cost': 15000,
                'bandwidth_efficiency': 0.95,
                'security_level': 'High',
    ...
    def analyze_network_transfer_with_claude(self, migration_params: Dict) -> Dict:
        """Generate AI-powered network transfer recommendations using Claude API"""
        
        if not self.client:
            return self._get_fallback_analysis(migration_params)
        
        try:
            # Prepare context for Claude
            context = self._prepare_network_context(migration_params)
            
            # Create the prompt for Claude
            prompt = f"""
            You are an AWS network architecture expert specializing in database migration data transfer patterns.
            
            Analyze this migration scenario and provide specific network transfer recommendations:
            
            {context}
            
            Please provide a comprehensive analysis with:
            
            1. PRIMARY RECOMMENDATION: Choose the best transfer pattern and explain why
            2. COST ANALYSIS: Break down costs for your recommended approach
            3. TIMELINE IMPACT: How this affects the migration schedule
            4. RISK ASSESSMENT: Key risks and mitigation strategies
            5. ALTERNATIVE OPTIONS: 2-3 backup options with trade-offs
            6. IMPLEMENTATION STEPS: Specific technical steps to implement
            
            Be specific about:
            - Exact AWS services to use
            - Cost estimates based on the data size and requirements
            - Timeline for setup and data transfer
            - Technical configuration considerations
            - Security implications
            
            Format your response as structured recommendations that can guide implementation decisions.
            """
            ...
            structured_analysis = self._parse_claude_response(ai_response, migration_params)
            structured_analysis['source'] = 'Claude AI'
            structured_analysis['api_success'] = True
            structured_analysis['full_response'] = ai_response
            
            return structured_analysis
            
        except Exception as e:
            print(f"Claude API call failed: {e}")
            fallback = self._get_fallback_analysis(migration_params)
            fallback['api_error'] = str(e)
            return fallback
    
    def _prepare_network_context(self, migration_params: Dict) -> str:
        ...
        - Data Size: {data_size_gb:,} GB ({data_size_tb:.1f} TB)
        - Source Database: {migration_params.get('source_engine', 'Unknown')}
        - Target Database: {migration_params.get('target_engine', 'Unknown')}
        - AWS Region: {migration_params.get('region', 'us-east-1')}
        - Migration Timeline: {migration_params.get('migration_timeline_weeks', 12)} weeks
        - Available Bandwidth: {migration_params.get('bandwidth_mbps', 1000)} Mbps
        - Team Expertise: {migration_params.get('team_expertise', 'Intermediate')}
        
        BUSINESS CONSTRAINTS:
        - Budget Level: {migration_params.get('budget_constraints', 'Medium')}
        - Security Requirements: {migration_params.get('security_requirements', 'Standard')}
        - Downtime Tolerance: {migration_params.get('downtime_tolerance', 'Medium')}
        - Compliance Needs: {migration_params.get('compliance_requirements', 'Standard')}
        
        NETWORK INFRASTRUCTURE:
        - Direct Connect Available: {migration_params.get('has_direct_connect', False)}
        - VPN Capability: {migration_params.get('has_vpn', True)}
        - Network Redundancy: {migration_params.get('network_redundancy', 'Standard')}
        - Internet Bandwidth: {migration_params.get('internet_bandwidth_mbps', 1000)} Mbps
        
        AVAILABLE TRANSFER PATTERNS:
        """
        
        # Add transfer pattern details for Claude's reference
        for pattern_id, pattern_info in self.transfer_patterns.items():
            context += f"""
        - {pattern_info['name']}: {pattern_info['description']}
          Cost: ~${pattern_info['typical_cost_per_gb']}/GB, Setup: ${pattern_info['setup_cost']:,}
          Security: {pattern_info['security_level']}, Complexity: {pattern_info['complexity']}
            """
        
        # Add calculated estimates for Claude's context
        internet_transfer_days = self._estimate_transfer_time(data_size_gb, migration_params.get('bandwidth_mbps', 1000) * 0.7)
        dx_transfer_days = self._estimate_transfer_time(data_size_gb, migration_params.get('bandwidth_mbps', 1000) * 0.95)
        
        context += f"""
        
        TRANSFER TIME ESTIMATES:
        - Internet Transfer: ~{internet_transfer_days:.1f} days
        - Direct Connect Transfer: ~{dx_transfer_days:.1f} days
        - Available Transfer Window: {migration_params.get('migration_timeline_weeks', 12) * 7 * 0.3:.0f} days (30% of timeline)
        """
        
        return context
    
    def _estimate_transfer_time(self, data_size_gb: int, effective_bandwidth_mbps: float) -> float:
        """Estimate transfer time in days"""
        # Convert GB to bits, then calculate hours
        data_bits = data_size_gb * 8 * 1024 * 1024 * 1024  # GB to bits
        bandwidth_bps = effective_bandwidth_mbps * 1024 * 1024  # Mbps to bps
        transfer_hours = data_bits / bandwidth_bps / 3600  # Convert to hours
        return transfer_hours / 24  # Convert to days
    
    def _parse_claude_response(self, ai_response: str, migration_params: Dict) -> Dict:
        ...
        # Parse sections based on headers/keywords
        current_section = None
        
        for section in sections:
            section_lower = section.lower().strip()
            
            if 'primary recommendation' in section_lower:
                structured['primary_recommendation'] = section
                current_section = 'primary'
            elif 'cost analysis' in section_lower:
                structured['cost_analysis'] = section
                current_section = 'cost'
            elif 'timeline' in section_lower:
                structured['timeline_impact'] = section
                current_section = 'timeline'
            elif 'risk' in section_lower:
                structured['risk_assessment'] = section
                current_section = 'risk'
            elif 'alternative' in section_lower or 'option' in section_lower:
                structured['alternative_options'].append(section)
                current_section = 'alternatives'
            elif 'implementation' in section_lower or 'steps' in section_lower:
                ...
                structured['implementation_steps'] = steps
                current_section = 'implementation'
        
        # Extract key insights (recommendations, important points)
        insights = []
        for section in sections:
            lines = section.split('\n')
            for line in lines:
                ...
                if (line.startswith('•') or line.startswith('-') or 
                    'recommend' in line.lower() or 'should' in line.lower()):
                    insights.append(line.lstrip('•- '))
        
        structured['key_insights'] = insights[:8]  # Top 8 insights
        
        # Add calculated metrics for consistency
        structured['estimated_costs'] = self._extract_cost_estimates(ai_response, migration_params)
        structured['recommended_pattern'] = self._extract_recommended_pattern(ai_response)
        
        return structured
    
    def _extract_cost_estimates(self, ai_response: str, migration_params: Dict) -> Dict:
        """Extract cost estimates from Claude's response"""
        
        # Look for cost mentions in the response
        import re
        
        cost_pattern = r'\$[\d,]+(?:\.\d{2})?'
        costs = re.findall(cost_pattern, ai_response)
        
        # Calculate basic estimates as fallback
        data_size_gb = migration_params.get('data_size_gb', 1000)
        
        return {
            'setup_cost_range': '2,000 - 15,000',
            'data_transfer_cost_range': f'{data_size_gb * 0.01:,.0f} - {data_size_gb * 0.09:,.0f}',
            'monthly_infrastructure': '500 - 2,000',
            'mentioned_costs': costs[:5] if costs else []
        }
    
    def _extract_recommended_pattern(self, ai_response: str) -> str:
        """Extract the recommended pattern from Claude's response"""
        ...
        for pattern_id, pattern_info in self.transfer_patterns.items():
            ...
            if pattern_name_lower in response_lower:
                return pattern_info['name']
        
        # Look for service mentions
        if 'direct connect' in response_lower and 'dms' in response_lower:
            return 'Direct Connect + DMS'
        elif 'snowball' in response_lower:
            return 'Snowball + DMS Hybrid'
        elif 'vpn' in response_lower:
            return 'VPN + DMS'
        elif 'datasync' in response_lower:
            return 'Direct Connect + DataSync + VPC'
        else:
            return 'Internet + DMS'
    
    def _get_fallback_analysis(self, migration_params: Dict) -> Dict:
        """Provide intelligent fallback analysis when Claude API is unavailable"""
        ...
        has_dx = migration_params.get('has_direct_connect', False)
        
        # Smart fallback logic
        if data_size_gb > 10000:  # >10TB
            recommended_pattern = 'Snowball + DMS Hybrid'
            reasoning = f"Large data volume ({data_size_gb:,} GB) makes Snowball cost-effective for initial transfer"
        elif has_dx and data_size_gb > 1000:
            recommended_pattern = 'Direct Connect + DMS'
            reasoning = "Existing Direct Connect infrastructure provides reliable, cost-effective transfer"
        elif budget_level == 'low' or data_size_gb < 500:
            recommended_pattern = 'Internet + DMS'
            reasoning = "Moderate data size and budget constraints favor internet-based transfer"
        else:
            recommended_pattern = 'VPN + DMS'
            reasoning = "Balanced approach providing security and reasonable setup costs"
        
        return {
            'primary_recommendation': f"""
            RECOMMENDED PATTERN: {recommended_pattern}
            
            REASONING: {reasoning}
            
            This recommendation is based on algorithmic analysis of your data size, timeline, 
            and infrastructure constraints. For AI-powered insights, please provide an Anthropic API key.
            """,
            'cost_analysis': f"""
            ESTIMATED COSTS:
            - Data Size: {data_size_gb:,} GB
            - Setup Cost: ${self._estimate_setup_cost(recommended_pattern):,}
            - Transfer Cost: ${self._estimate_transfer_cost(data_size_gb, recommended_pattern):,}
            - Timeline: {timeline_weeks} weeks
            """,
            'timeline_impact': f"Estimated transfer time: {self._estimate_pattern_timeline(recommended_pattern, data_size_gb)} days",
            'risk_assessment': "Medium risk - standard AWS migration approach with established patterns",
            'alternative_options': ["Direct Connect + DMS (if higher performance needed)", "Internet + DMS (if cost optimization required)"],
            'implementation_steps': self._get_pattern_steps(recommended_pattern),
            'key_insights': [
                f"Data volume of {data_size_gb:,} GB influences pattern selection",
                f"Timeline of {timeline_weeks} weeks provides adequate buffer",
                "Consider security and compliance requirements in final selection",
                "Test connectivity and performance before full migration"
            ],
    ...
    def _estimate_setup_cost(self, pattern: str) -> int:
        """Estimate setup cost for pattern"""
        cost_map = {
            'Internet + DMS': 2000,
            'Direct Connect + DMS': 8000,
            'VPN + DMS': 3000,
            'Snowball + DMS Hybrid': 15000,
            'Direct Connect + DataSync + VPC': 12000
        }
        return cost_map.get(pattern, 5000)
    
    def _estimate_transfer_cost(self, data_size_gb: int, pattern: str) -> int:
        """Estimate transfer cost for pattern"""
        cost_per_gb_map = {
            'Internet + DMS': 0.09,
            'Direct Connect + DMS': 0.02,
            'VPN + DMS': 0.09,
            'Snowball + DMS Hybrid': 0.01,
            'Direct Connect + DataSync + VPC': 0.0125
        }
        cost_per_gb = cost_per_gb_map.get(pattern, 0.05)
        return int(data_size_gb * cost_per_gb)
    
    def _estimate_pattern_timeline(self, pattern: str, data_size_gb: int) -> int:
        """Estimate timeline in days for pattern"""
        # Simple estimation - would be more sophisticated in production
        base_days = data_size_gb / 1000  # 1 day per TB baseline
        
        if 'Snowball' in pattern:
            return max(7, base_days * 0.5)  # Faster but minimum shipping time
        elif 'Direct Connect' in pattern:
            return max(1, base_days * 0.8)  # Faster than internet
        else:
            return max(2, base_days)  # Internet baseline
    
    def _get_pattern_steps(self, pattern: str) -> List[str]:
        ...
        steps_map = {
            ...
            'Direct Connect + DMS': [
                "Verify Direct Connect connectivity",
                "Configure DMS over Direct Connect",
                "Set up database endpoints",
                "Test performance",
                "Execute migration"
            ],
            'VPN + DMS': [
                "Establish VPN connection",
                "Configure DMS instance",
                "Set up endpoints over VPN",
                "Test connectivity",
                "Execute migration"
            ],
            'Snowball + DMS Hybrid': [
                "Order Snowball devices",
                "Extract data to Snowball",
                "Ship and import to S3",
                "Set up DMS for ongoing sync",
                "Coordinate cutover"
            ]
        }
        return steps_map.get(pattern, ["Contact AWS support for pattern-specific guidance"])


# Usage example for Streamlit integration:
def analyze_network_transfer_claude(migration_params: Dict, anthropic_api_key: Optional[str] = None) -> Dict:
    """
    Main function to get AI-powered network transfer recommendations
    
    Args:
        migration_params: Dictionary containing migration parameters
        anthropic_api_key: Optional Anthropic API key for Claude integration
    
    Returns:
        Dictionary with comprehensive network transfer analysis
    """
    
    analyzer = ClaudeNetworkTransferAnalyzer(anthropic_api_key)
    return analyzer.analyze_network_transfer_with_claude(migration_params)


# Integration example for your existing Streamlit code:
def show_claude_network_analysis():
    """Show Claude-powered network analysis in Streamlit"""
    
    st.markdown("## 🤖 AI-Powered Network Transfer Analysis")
    
    # Get API key from migration params
    anthropic_key = st.session_state.migration_params.get('anthropic_api_key')
    
    if not anthropic_key:
        st.warning("⚠️ Provide Anthropic API key for genuine AI analysis")
        st.info("Without API key, you'll get algorithmic recommendations")
    
    if st.button("🚀 Generate Claude Network Recommendations", type="primary"):
        
        with st.spinner("🤖 Claude AI analyzing your network transfer requirements..."):
            
            # Get genuine Claude analysis
            network_analysis = analyze_network_transfer_claude(
                st.session_state.migration_params, 
                anthropic_key
            )
            
            # Store results
            st.session_state.network_analysis = network_analysis
            
            # Show API status
            if network_analysis.get('api_success'):
                st.success("✅ Analysis completed using Claude AI")
            else:
                st.warning("⚠️ Using fallback analysis (Claude API unavailable)")
            
            # Display results
            st.markdown("### 🎯 Primary Recommendation")
            st.write(network_analysis['primary_recommendation'])
            
            st.markdown("### 💰 Cost Analysis")
            st.write(network_analysis['cost_analysis'])
            
            st.markdown("### ⏱️ Timeline Impact")
            st.write(network_analysis['timeline_impact'])
            
            if network_analysis.get('implementation_steps'):
                st.markdown("### 🛠️ Implementation Steps")
                for i, step in enumerate(network_analysis['implementation_steps'], 1):
                    st.write(f"{i}. {step}")
